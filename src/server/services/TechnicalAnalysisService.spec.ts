  ...
  describe('analyzeTechnicalIndicators', () => {
    ...
    describe('signal interpretation', () => {
      ...
      it('should interpret Stochastic overbought as bearish', async () => {
        ...
        expect(result.indicators.momentum?.stochastic?.signal).toBe(
          'overbought',
        );
      });
      ...
      it('should interpret Williams %R overbought (>-20) as bearish', async () => {
        ...
        expect(result.indicators.momentum?.williamsR?.signal).toBe(
          'overbought',
        );
      });
      ...
      it('should interpret swing points downtrend', async () => {
        ...
        expect(result.indicators.patterns?.swingPoints?.trend).toBe(
          'downtrend',
        );
      });
      ...
      it('should handle pivot points woodie type', async () => {
        ...
        expect(result.indicators.supportResistance?.pivotPoints?.pivot).toBe(
          100,
        );
      });
    ...
    describe('edge cases and null handling', () => {
      ...
      it('should handle fetchDailyCandles with null candles response', async () => {
        ...
        expect(
          result.indicators.supportResistance?.pivotPoints,
        ).toBeUndefined();
      });
    });

    describe('risk metrics', () => {
      // Helper to generate candles with specific close prices for predictable risk calculations
      const generateCandlesWithPrices = (
        prices: number[],
      ): Array<{
        start: number;
        low: number;
        ...
        return prices
          .map((price, index) => ({
            start: Date.now() / 1000 - (prices.length - index) * 3600,
            low: price * 0.99,
            high: price * 1.01,
            open: price,
            close: price,
      ...
      it('should calculate risk metrics from candle data', async () => {
        // Generate enough candles for risk calculation (at least 2 for log returns)
        const prices = Array.from({ length: 30 }, (_, i) => 100 + i);
        const mockCandles = generateCandlesWithPrices(prices);
        getProductCandlesMock.mockResolvedValue({
          candles: mockCandles,
        });
        ...
        expect(result.risk?.volatilityDaily).toBeGreaterThanOrEqual(0);
        expect(result.risk?.volatilityAnnualized).toBeGreaterThanOrEqual(0);
        expect(result.risk?.var95).toBeDefined();
        expect(result.risk?.maxDrawdown).toBeGreaterThanOrEqual(0);
        expect(result.risk?.sharpeRatio).toBeDefined();
        expect(['low', 'moderate', 'high', 'extreme']).toContain(
          result.risk?.riskLevel,
        );
      });

      it('should calculate correct volatility from known data', async () => {
        // Prices [100, 110, 100] give known log returns:
        // ln(110/100) = 0.09531 and ln(100/110) = -0.09531
        // Mean = 0, so std dev = 0.09531 (daily volatility)
        ...
        expect(result.risk).toBeDefined();
        // Daily volatility should be approximately 0.0953 (std dev of log returns)
        expect(result.risk?.volatilityDaily).toBeCloseTo(0.0953, 3);
        // Annualized = daily * sqrt(8760) for hourly granularity
        expect(result.risk?.volatilityAnnualized).toBeCloseTo(
          0.0953 * Math.sqrt(8760),
          1,
        );
      });

      it('should calculate VaR at 95% confidence', async () => {
        ...
        expect(result.risk).toBeDefined();
        // VaR should be positive (represents potential loss as positive number)
        expect(result.risk?.var95).toBeGreaterThan(0);
      });

      it('should calculate max drawdown correctly', async () => {
        // Prices [100, 120, 90, 110] where max drawdown is 25% (120 -> 90)
        const prices = [100, 120, 90, 110];
        const mockCandles = generateCandlesWithPrices(prices);
        getProductCandlesMock.mockResolvedValue({
          candles: mockCandles,
        });
        ...
        expect(result.risk).toBeDefined();
        // Max drawdown from peak 120 to trough 90 = (120-90)/120 = 0.25 (25%)
        expect(result.risk?.maxDrawdown).toBeCloseTo(0.25, 2);
      });

      it('should return null sharpeRatio when volatility is zero', async () => {
        // Constant prices result in zero volatility
        ...
        expect(result.risk).toBeDefined();
        expect(result.risk?.volatilityDaily).toBe(0);
        expect(result.risk?.sharpeRatio).toBeNull();
      });

      it('should classify risk levels correctly', async () => {
        // Test each threshold boundary based on annualized volatility
        // Low: < 30%, Moderate: 30-60%, High: 60-100%, Extreme: >= 100%

        // For ONE_DAY granularity, periodsPerYear = 365
        // To get annualized volatility of X%, we need daily volatility of X% / sqrt(365)

        // Test low risk (< 30% annualized)
        // Daily vol needed: 0.29 / sqrt(365) ≈ 0.0152
        // Log return for 1.5% daily change: ln(1.015) ≈ 0.0149
        const lowRiskPrices = [100, 101.5, 100];
        getProductCandlesMock.mockResolvedValue({
          candles: generateCandlesWithPrices(lowRiskPrices),
        });

        let result = await service.analyzeTechnicalIndicators({
          productId: 'BTC-USD',
          granularity: Granularity.ONE_DAY,
          indicators: [],
        });
        ...
        // Test extreme risk (>= 100% annualized)
        // Daily vol needed: 1.0 / sqrt(365) ≈ 0.0523
        // But we're using hourly, so periodsPerYear = 8760
        // Hourly vol needed: 1.0 / sqrt(8760) ≈ 0.0107
        // But for easier testing, use dramatic price swings
        const extremeRiskPrices = [100, 200, 50, 150, 75];
        getProductCandlesMock.mockResolvedValue({
          candles: generateCandlesWithPrices(extremeRiskPrices),
        });

        result = await service.analyzeTechnicalIndicators({
          productId: 'BTC-USD',
          granularity: Granularity.ONE_HOUR,
          indicators: [],
        });
        expect(result.risk?.riskLevel).toBe('extreme');
      });

      it('should return undefined risk when insufficient candles', async () => {
        // Only 1 candle - not enough to calculate log returns
        const mockCandles = generateCandlesWithPrices([100]);
        getProductCandlesMock.mockResolvedValue({
          candles: mockCandles,
        });
      ...
      it('should handle zero prices gracefully', async () => {
        // Zero prices cause log returns to be skipped, resulting in empty returns
        const mockCandles = [
          {
            start: Date.now() / 1000,
            low: 0,
          ...
          },
          {
            ...
            high: 0,
            open: 0,
            close: 0, // Zero price - log return will be skipped
            volume: 1000,
          },
        ];
        getProductCandlesMock.mockResolvedValue({
        ...
        // Risk should be undefined when log returns are empty
        expect(result.risk).toBeUndefined();
      });
    });
  });
