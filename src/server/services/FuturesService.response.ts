import { z } from 'zod';
import { nullToUndefined, stringToNumber } from './schema.helpers';
...
export const GetFuturesPositionResponseSchema = z
  .object({
    position: nullToUndefined(FCMPositionSchema).describe('Position details'),
  })
...
export const GetFuturesBalanceSummaryResponseSchema = z
  .object({
    balanceSummary: nullToUndefined(FCMBalanceSummarySchema).describe(
      'Balance summary',
    ),
  })
