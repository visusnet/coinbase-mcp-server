...
export const CandleInputSchema = z.object({
  open: z.preprocess((v) => Number(v) || 0, z.number()).describe('Open price'),
  high: z.preprocess((v) => Number(v) || 0, z.number()).describe('High price'),
 ...
 */
export const CandleInputArraySchema = z.array(CandleInputSchema);

/**
 * L2 Level schema for order book data.
 */
const L2LevelResponseSchema = z.object({
  price: stringToNumber.describe('Price level'),
  size: stringToNumber.describe('Size at price level'),
});

export type L2Level = z.output<typeof L2LevelResponseSchema>;
...
export const PriceBookSchema = z.object({
  ...
  bids: z.array(L2LevelResponseSchema).describe('Bid levels'),
  asks: z.array(L2LevelResponseSchema).describe('Ask levels'),
 ...
 */
export const HistoricalMarketTradeSchema = z.object({
  ...
  bid: z.string().optional().describe('Bid'),
  ask: z.string().optional().describe('Ask'),
});
...
export const ProductSchema = z.object({
  ...
  pricePercentageChange24h:
    stringToNumberRequired.describe('24h price change %'),
  volume24h: stringToNumberRequired.describe('24h volume'),
  volumePercentageChange24h: stringToNumberRequired.describe(
    '24h volume change %',
  ),
